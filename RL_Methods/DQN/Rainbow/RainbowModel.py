...
class RainbowModel(DuelingModel):
    ...
    def _make_network(self, achitecture, input_dim, output_dim, device):
        ...
        for i in range(len(feature_arch)):
            feature_extractor.add_module("layer_{}".format(i+1), NoisyFactorizedLinear(last_dim, feature_arch[i], self.initial_sigma, bias=True))
        ...
        advantage_net.add_module("ouput", NoisyFactorizedLinear(last_dim, output_dim * self.n_atoms, self.initial_sigma, bias=True))
